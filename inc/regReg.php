    ...
    if($userInfo){
        ...
        if($password){
            if((strlen($password) > 6 and strlen($password) < 16) and ($password == $password2)){
                if(preg_match($upperCaseRegex,$password) and preg_match($lowerCaseRegex,$userInfo['password']) and !preg_match($rusCaseRegex,$password) and preg_match($numberRegex,$password) and preg_match($specialCharRegex,$password)) $passwordreg = password_hash($password, PASSWORD_DEFAULT);
            }else $passError = true;
        }
    }
    if(isPOST()){
